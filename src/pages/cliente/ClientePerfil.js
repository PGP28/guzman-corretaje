...
/* ── Token del cliente logueado ── */
const getToken = () => localStorage.getItem('guzman_cliente_token');

/* ══════════════════════════════════════════════
   Subcomponente interno con acceso al hook de Google
══════════════════════════════════════════════ */
const PerfilForm = ({ user, onActualizar }) => {
  ...
  /* ── Conectar Gmail via Google OAuth ── */
  ...
  const guardar = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('guzman_cliente', JSON.stringify({
        ...clienteLocal,
        email: data.email || clienteLocal.email,
      }));
      onActualizar?.(data);
    ...
    <div className="cpf-page">
      ...
      <div className="cpf-header">
        <div className="cpf-avatar-wrap">
          {foto
            ? <img src={foto} alt={perfil?.nombre} className="cpf-avatar" />
            : <div className="cpf-avatar-placeholder">{inicial}</div>
          }
          {/* Indicador de perfil completo */}
          {tieneGmail && tieneTel && (
            <span className="cpf-avatar-badge" title="Perfil completo">✓</span>
          )}
        </div>
        <div>
      ...
      {/* ── Banner si falta info importante ── */}
      {(!tieneGmail || !tieneTel) && !editando && (
        <div className="cpf-banner">
          ...
          <div>
            ...
            <p>
              Agrega tu {!tieneTel && 'teléfono'}{!tieneTel && !tieneGmail && ' y '}
              {!tieneGmail && 'Gmail'} para que tu corredor pueda coordinar visitas
              y reuniones por Google Meet contigo.
            </p>
          </div>
          <button className="cpf-banner-btn" onClick={() => setEditando(true)}>
            Completar ahora
          </button>
        </div>
      )}
      ...
      {!editando && (
        <div className="cpf-card">
          <div className="cpf-card-header">
            ...
            <button className="cpf-btn-editar" onClick={() => setEditando(true)}>
              <FaEdit /> Editar
            </button>
          </div>
          ...
          <div className="cpf-info-grid">
            ...
            <div className="cpf-info-item">
              ...
              <div>
                <span className="cpf-info-label">Nombre completo</span>
                <span className="cpf-info-valor">{perfil?.nombre || '—'}</span>
              </div>
            </div>
            ...
            <div className={`cpf-info-item ${!tieneTel ? 'cpf-info-item--vacio' : ''}`}>
              ...
              <div>
                <span className="cpf-info-label">Teléfono</span>
                <span className="cpf-info-valor">
                  {tieneTel ? perfil.telefono : <em className="cpf-sin-dato">Sin teléfono — agrega uno</em>}
                </span>
              </div>
            ...
            <div className={`cpf-info-item ${!tieneGmail ? 'cpf-info-item--vacio' : ''}`}>
              ...
              <div>
                <span className="cpf-info-label">Gmail (para Meet)</span>
                <span className="cpf-info-valor">
                  {tieneGmail ? perfil.email : <em className="cpf-sin-dato">Sin Gmail — conéctalo para reuniones</em>}
                </span>
              </div>
          ...
          {exito && <div className="cpf-exito">{exito}</div>}
        </div>
      )}
        ...
        <div className="cpf-card">
          ...
          <form onSubmit={guardar} className="cpf-form">
            ...
            <div className="cpf-campo">
              <label htmlFor="tel">
                <FaPhone className="cpf-label-icon" /> Teléfono
              </label>
              <input
                id="tel"
                type="tel"
                ...
                autoComplete="tel"
              />
              <small className="cpf-campo-hint">
                Tu corredor lo usará para coordinar visitas.
              </small>
            </div>

            {/* Gmail — con opción de conectar Google */}
            <div className="cpf-campo">
              <label htmlFor="gmail">
                <FaGoogle className="cpf-label-icon cpf-label-icon--google" /> Gmail
              </label>
              ...
              <div className="cpf-gmail-row">
                <input
                  ...
                  className="cpf-input cpf-input--gmail"
                  autoComplete="email"
                />
                <button
                  type="button"
                  className="cpf-btn-google"
                  onClick={() => conectarGoogle()}
                  title="Autocompletar con Google"
                >
              ...
              <small className="cpf-campo-hint">
                Solo Gmail (@gmail.com). Se usará para enviarte invitaciones a Google Meet.
              </small>
            </div>
            ...
            <div className="cpf-form-btns">
              <button type="button" className="cpf-btn-cancelar" onClick={cancelar}>
                Cancelar
              </button>
      ...
      <div className="cpf-card cpf-card--info">
        ...
        <div className="cpf-beneficios">
          <div className="cpf-beneficio">
            <span className="cpf-beneficio-icon">📞</span>
            <div>
              <strong>Teléfono</strong>
              <p>Tu corredor puede llamarte o enviarte WhatsApp para coordinar visitas a propiedades.</p>
            </div>
          </div>
          <div className="cpf-beneficio">
            <span className="cpf-beneficio-icon">📹</span>
            <div>
              <strong>Gmail y Google Meet</strong>
              <p>Si prefieres una reunión virtual, tu corredor puede enviarte una invitación a Google Meet directamente a tu Gmail.</p>
            </div>
          </div>
          <div className="cpf-beneficio">
            <span className="cpf-beneficio-icon">🔒</span>
            <div>
              ...
              <p>Tu información solo es visible para tu corredor asignado. Nunca se comparte con terceros.</p>
            </div>
          </div>
        </div>
      </div>
...
/* ══════════════════════════════════════════════
   Componente exportado (envuelto en GoogleOAuthProvider)
══════════════════════════════════════════════ */
